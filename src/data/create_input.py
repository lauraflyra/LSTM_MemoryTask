...
# Input has shape (batch size, Total time steps,  input size)
...
output = np.zeros((BATCH_SIZE, TOTAL_TIME_STEPS, INPUT_SIZE))  # Output is gonna be in one-hot encoding of people
for batch_item, i in enumerate(go_signal_idx):
    output[batch_item, go_signal_moments[batch_item], i * 4:i * 4 + 4] = x[batch_item, CUE_TIME, i * 4:i * 4 + 4]

x = x.reshape((TOTAL_TIME_STEPS, BATCH_SIZE, INPUT_SIZE))

output = output.reshape((TOTAL_TIME_STEPS, BATCH_SIZE, INPUT_SIZE))